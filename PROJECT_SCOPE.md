...
Каноническая политика Claude/Codex находится в `AGENTS.md`. Claude/Codex владеют
контрактами, permissions, интеграцией и финальной проверкой; нативные агенты
получают изолированные repository scopes, а routine work по умолчанию идёт малыми
ограниченными ClinePass-вызовами. Актуальные ClinePass-модели и маршруты хранятся
только в `tools/clinepass-mcp/models.env`, секреты — только в игнорируемом
`tools/clinepass-mcp/.env`.
...
> HTML-мокапы для P0: `ui/03 Карточка инцидента/`, `ui/02 Живой мониторинг/`, `ui/05 Интерактивный отчёт/`
