...
Каноническая политика Claude/Codex находится в `AGENTS.md`. Claude/Codex работают
напрямую и максимально используют нативных агентов; ClinePass остаётся
необязательным вспомогательным слоем для подходящих простых, массовых или
самодостаточных задач. Актуальные ClinePass-модели и маршруты хранятся только в
`tools/cline-mcp/models.env`, секреты — только в игнорируемом
`tools/cline-mcp/.env`.
