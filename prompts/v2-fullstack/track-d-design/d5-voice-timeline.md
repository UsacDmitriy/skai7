...
- `Timeline`: линия трека `#1E3A8A`; точки окрашены по severity (маппинг d1); маркер `t=0` — `critical`
  и визуально выделен; клик по точке вызывает `onSelect` с этим событием; значок видео виден у `has_video`.
