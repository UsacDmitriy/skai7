...
Единая схема: **Барьер 0 → Волна 1 → Барьер 1 → Волна 2 → Барьер 2 → Волна 3 → Барьер 3** (подробности и команды — в `EXECUTION.md`).
...
6. **Волна 3 — бэклог + тест-хардненинг.** Неблокирующие правки из аудитов (W3-1/W3-2) и
...
7. **Барьер 3 — хардненинг.** `x5-wave3-hardening` — полный регресс (unit+API+фронт) + гейт покрытия
