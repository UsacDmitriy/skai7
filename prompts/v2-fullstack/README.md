...
7. **Волна 4 — AI-слой (идеи #11–#20).** Новое поверх готового продукта; внешние API/VLM — **офлайн-предрасчёт → кэш**.
   - **4.1 Умное событие + прогнозы** — `b16` сцена-по-кадру (VLM) → `b17` кросс-проверка Open-Meteo/sunrise + риск ‖ `b18` прогноз (ARIMA+IForest) ‖ `b19` геозоны (DBSCAN+РЭБ) ‖ `b20` цепочки усталости ‖ `b24` governance (флаги/latency/cache) ‖ `d7` ‖ `t5` CURRENT_STATUS ‖ `t6` remote-CI+live-smoke → **Барьер 4.1** (`x6` smoke).
   - **4.2 Ассистент + визуализация** — `b21` Fleet Copilot (tool-use, RU/EN) ‖ `b22` нарратив ‖ `b23` умный вердикт саботажа ‖ `b25` метрики/data-quality ‖ `b26` security-baseline ‖ `f15`–`f21` (сцена/прогноз/копилот/heatmap/вердикт/risk-waterfall/metrics) → **Барьер 4.2** (`x7` e2e) → merge в `main`.
   - **Слой измеримости/explainability (по 2-му research-отчёту, #17–#20):** governance `b24`, метрики+data-quality `b25`/`f21`, risk-waterfall `f20`, hardening `t5`/`t6`/`b26`.
