...
Единая схема: **Барьер 0 → Волна 1 → Барьер 1 → Волна 2 → Барьер 2 → Волна 3 (бэклог)** (подробности и команды — в `EXECUTION.md`).
...
6. **Волна 3 — бэклог некритичных доработок.** Неблокирующие правки/улучшения из аудитов
   (`wave-3-backlog/`). Без отдельного барьера: каждый пункт идёт на ветке трека-владельца и
   приезжает обычным мержем. Очередь и правила — в `wave-3-backlog/README.md`.
