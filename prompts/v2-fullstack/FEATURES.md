...
> P0-промпты Волны 1 (`b2`/`f4`/`b3`/`d2`) уже выполнены → их DoD/owner-only доработка вынесена в `b14`/`f14` и `b15`/`d6` (Волна 2.1, поверх готового).
...
- **Реализация глубины:** базовый экран — `f4` (выполнен, Волна 1); состояния/sync/a11y/локали — `wave-2-1-reports-voice/track-f-frontend/f14-incidentcard-hardening`; робастность синк-примитивов (троттлинг/cleanup/петли) — `track-d-design/d6-sync-hardening`; целостность спайна `v_incidents` — `track-b-backend/b15-vincidents-hardening` (всё owner-only у Claude/Codex, Волна 2.1, поверх готового).
