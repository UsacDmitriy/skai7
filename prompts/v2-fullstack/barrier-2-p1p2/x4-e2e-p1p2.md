...
> **Модель:** 🔴 Opus — кросс-файловая интеграция / анти-регресс / killer-feature / барьер.
> трек T `prompts/v2-fullstack/track-t-tests/t2-api-integration-tests` (`test_reports_api`, `test_p1p2_api`) и `t3`.
> Ничего не переписывает — при провале заводит дефект соответствующему треку (b7–b13 / d4,d5 / f5–f13 / t1–t4).
> Запускается **после Барьеров 2.1 (x4a) и 2.2 (x4b)** и завершения Волны 2.3 (тесты). Это финальный
> барьер Волны 2 — единственный, кто продвигает `main`.

## Перед стартом — подтянуть тесты (2.3)

В окне `skai_7` на ветке `integration`. **`main` = стабильный P0 (после x3) — не трогаем**, пока P1/P2-smoke не зелёный.

Срезы 2.1/2.2 (b7–b13, d4/d5, f5–f13) уже в `integration` (барьеры x4a/x4b + x2-rewire). Перед финалом
