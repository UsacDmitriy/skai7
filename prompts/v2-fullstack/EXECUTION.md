...
> x4 для P1/P2, x5 для Волны 3, **x8 для Волны 4** — финал, x9 для Волны 4.4). Барьеры x6/x7 `main` не трогают (smoke/e2e на
...
> Файлы барьеров (по одному на волну): `barrier-1-p0/` (x1–x3) · `barrier-2-1-reports-voice/` (x4a) · `barrier-2-2-applied/` (x4b) · `barrier-2-3-tests/` (x4) · `barrier-3-hardening/` (x5) · `barrier-4-1-smart-context/` (x6) · `barrier-4-2-assistant/` (x7) · `barrier-4-3-ops-trust/` (x8) · `barrier-4-4-data-trust/` (x9). Общие `x2`/`x3` живут в `barrier-1-p0/` и переиспользуются.
> **GUARD во всех барьерах, сливающих `feat/*`** (`x1`,`x2`,`x4a`,`x4b`,`x4`,`x5`,`x6`,`x7`,`x8`,`x9`): стоп при незакоммиченном worktree.
> **Все барьеры (`x1`–`x9`) — 🔴 Opus** (судят green/red, продвигают `main`, заводят дефекты); см. легенду моделей выше.
