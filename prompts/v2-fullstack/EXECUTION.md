...
> P0-промпты Волны 1 (`b2`,`f4`,`b3`,`d2`) уже выполнены, поэтому их owner-only доработка вынесена в отдельные
> промпты **`b14`/`f14` и `b15`/`d6` (Волна 2.1)** — правка поверх готового кода, а не переисполнение Волны 1.

## Обязательная политика исполнения

До запуска любой волны владелец проходит **Phase 0** из
[`tools/clinepass-mcp/README.md`](../../tools/clinepass-mcp/README.md): проверяет проектный мост,
Claude/Codex registration, unit/selftest, JSON/TOML, MCP `initialize` + `tools/list`, config,
audit reset/report, live model list, registry fallback и privacy boundary. Phase 0, контракты,
permissions, интеграция, deterministic verification, commit/push и финальный ответ принадлежат
Claude Opus/Codex Sol и не делегируются ClinePass.

Рутинное чтение репозитория, классификация, преобразование, черновой код/тесты и повторяемый review
по умолчанию выполняются малыми ограниченными ClinePass-вызовами. Нетривиальный пакет — это
**отдельные аудируемые вызовы** с общим `PACKAGE_ID`: planner → минимум два независимых worker →
reviewer → synthesizer. Один chat completion не может создавать native agents или вызывать MCP;
...
## Исторические теги сложности промптов

> Теги ниже сохранены как оценки сложности и blast radius, а не как runtime model routing:
> **🟢 routine-mechanical** — scaffold / токены / транскрипция / данные с полным структурным гейтом;
> **🔵 routine-deterministic** — ограниченная логика/вёрстка против контракта;
> **🔴 owner-only/high-judgement** — спайн данных, синк/алгоритм, security, сложный интерактив,
> кросс-экранное состояние, анти-регресс и все барьеры.
>
> **Правило эскалации:** если `## Check` routine-пакета дважды подряд красный, владелец забирает
> конкретный прогон как judgement/integration work. Тег не определяет ClinePass route или модель.

| Класс | Промпты |
| --- | --- |
| 🟢 routine-mechanical (8) | `b1`, `b4`, `d1`, `f1`, `f3`, `t4`, `w3-2`, `t5` (CURRENT_STATUS · W4.3) |
| 🔵 routine-deterministic (83) | **W1:** `b2`,`b5`,`b6`,`d3`,`f2` · **W2.1:** `b7`,`b8`,`b10`,`b14` · **W2.2:** `b11`,`b12`,`d4`,`f5`,`f8`,`f12` · **W2.3:** `t1`,`t2`,`t3`,`tu-*` (6) · **W3:** `w3-1`,`w3-3`,`w3-4`,`w3-5`,`w3-6`,`w3-7`,`w3-8`,`w3-10`,`w3-11`,`w3-12`,`w3-13`,`w3-14`,`w3-15`,`w3-16`,`w3-17`,`w3-18`,`w3-19` (w3-16…19 = prep В.4) · **W4.1:** `b17`,`b20`,`b24`,`d7`,`tu-scene/weather/forecast/zones/fatigue` · **W4.2:** `b22`,`b23`,`f15`,`f16`,`f19`,`tu-copilot`,`t-wave4-frontend` · **W4.3:** `b25`,`b27`,`f20`,`f21`,`f22`,`f23`,`f24`,`t6`,`tu-metrics/security/riskbreakdown` (f22/f23/f24 = целостность экранов) · **W4.4:** `b28`,`b29`,`f25`,`tu-consistency` (data trust §10) · **W5.1:** `b30`,`f26`,`tu-review` (§11) · **W5.2:** `b31`,`b32`,`f27`,`tu-coaching` (§12) · **W5.3:** `b33`,`b34`,`f28`,`tu-score` (§13) |
| 🔴 owner-only/high-judgement (37) | **фичи:** `b3` (спайн), `d2` (синк-примитивы), `f4` (флагман P0+синк), `b9` (двухпутевой NLU), `b13` (3 домена #5/#6/#7), `d5` (voice-UI killer #2), `f6` (дедуп+роли карты), `f7` (killer), `f9` (overlay/focus-trap/очередь), `f10` (таймлайн↔видео синк), `f11` (РЭБ-валидатор), `f13` (кросс-экранные роли), `f14` (анти-регресс #1) · **доработки Волны 1:** `b15`,`d6` · **W3:** `w3-9` (кросс-доменный join fleet-health) · **AI-слой (Волна 4):** `b16` (VLM-пайплайн), `b18` (прогноз-алгоритм), `b19` (DBSCAN-зоны), `b21` (copilot tool-use), `f17` (чат-UI), `f18` (heatmap-карта), `b26` (security-baseline · W4.3) · **барьеры:** `x1`,`x2`,`x3`,`x4`,`x4a`,`x4b`,`x5`,`x6`,`x7`,`x8`,`x9`,`x10`,`x11`,`x12` |

> Итог: **🟢 8 · 🔵 83 · 🔴 37** (128 промптов; +4 prep-промпта В.4 `w3-16…19`, +барьер `x8`; +полнота 4.3: `b27` + `tu-metrics/security/riskbreakdown`; +целостность экранов 4.3: `f22`/`f23`/`f24`; +Волна 4.4 Data Trust: `b28`/`b29`/`f25`/`tu-consistency` + `x9`; +Волна 5 (review/coaching/value): `b30`–`b34`,`f26`–`f28`,`tu-review/coaching/score` + `x10`–`x12`). Красный класс остаётся owner-only; зелёный и синий — кандидаты на bounded ClinePass packages с owner checks.
> **Волна 1 заморожена** (`b1`–`f4` выполнены) — её high-judgement глубина (по `b3`/`d2`) перенесена в доработки
...
**Один промпт = один owner package.** Владелец сначала отделяет owner-only gates, затем разбивает
нетривиальную рутинную часть на отдельные planner/worker/reviewer/synthesizer calls по обязательной
политике выше. В нужном окне открой панель Claude Code и дай команду:
...
>
> ⚠️ **Параллельные промпты в одном worktree — без `git add -A`.** Когда в одной ветке одновременно идут
> несколько промптов (`b18∥b19∥b20`, `f*∥f*`, `tu-*∥tu-*`), `git add -A` одного застейджит **недописанные
> файлы соседа** (так `w3-18` утянул фикстуры `w3-17`). Правило: `## Коммит` каждого промпта стейджит
> **только свои файлы по явному списку** (`git add <свои пути>`), не `-A`. Новые роутеры автодискаверятся
> (`api/main.py:_discover_routers`) — общий `api/routers/__init__.py` (`ALL_ROUTERS`, легаси) не редактируем.
...
- **Волна 4.4 — Data Trust** (backend → web ∥ tests, после зелёного x8): BACKEND **b28→b29** (последовательно: валидатор консистентности → кросс-сверка скоростей) ; WEB f25 (бейдж + панель) ; TESTS tu-consistency. Консистентность данных (§10, фичи #21/#22): кейсы Фомина (скорости) и Маслова (дубли терминалов).
- **Барьер 4.4 — Data Trust** (`skai_7`, `integration`): x9 (GUARD+merge → e2e #21/#22 + полный регресс) → merge в `main` (повторное продвижение, паттерн x8).
- **Волна 5.1 — Review Queue** (backend → web ∥ tests, после x9): BACKEND b30 ; WEB f26 (живой `/validation` — владелец сироты f22) ; TESTS tu-review. Очередь верификации (§11, фича #23): Фомин «39 → 5 подтверждённых» + паттерн Lytx.
- **Барьер 5.1** (`skai_7`, `integration`): x10 (GUARD+merge → e2e #23 + полный регресс) → `main` (паттерн x9).
- **Волна 5.2 — Coaching Loop** (backend → web ∥ tests, после x10): BACKEND **b31→b32** (генератор синтетики → сервис) ; WEB f27 (секция отчёта + бейдж «демо») ; TESTS tu-coaching. Цикл обучения Оздоева (§12, фича #24), repeat_within_30d — реальный расчёт.
- **Барьер 5.2** (`skai_7`, `integration`): x11 (GUARD+merge → e2e #24 + байт-стабильность генератора + полный регресс) → `main`.
- **Волна 5.3 — Driver Value** (backend → web ∥ tests, после x11): BACKEND **b33→b34** (позитив → единый рейтинг) ; WEB f28 (`/leaderboard` + блок позитива) ; TESTS tu-score. Green-zone Оздоева + Netradyne-паттерн (§13, фичи #25/#26); формулы зафиксированы контрактом.
- **Барьер 5.3 — финал Волны 5** (`skai_7`, `integration`): x12 (GUARD+merge → e2e #25/#26 + инвариант бленда + полный регресс) → `main`.
...
> x4 для P1/P2, x5 для Волны 3, **x8 для Волны 4** — финал, x9 для Волны 4.4, x10–x12 для подволн Волны 5). Барьеры x6/x7 `main` не трогают (smoke/e2e на
...
flowchart TD
    ...
    subgraph W42["ВОЛНА 4.2 · Ассистент + визуализация — окна 1 и 2"]
        direction LR
        subgraph B42["🪟 Окно 1 · backend (feat/backend) — ⚠ роутеры через автодискавери"]
            direction TB
    ...
    subgraph W44["ВОЛНА 4.4 · Data Trust — консистентность данных (§10)"]
        direction LR
        subgraph B44["🪟 Окно 1 · backend — ⚠ роутеры через автодискавери"]
            direction TB
            b28["b28 валидатор консистентности (7 проверок) · 🔵"] --> b29["b29 кросс-сверка скоростей · 🔵"]
        end
        subgraph F44["🪟 Окно 2 · web"]
            f25["f25 SpeedCheckBadge + ConsistencyPanel · 🔵"]
        end
        subgraph T44["🪟 Окно 3 · tests"]
            tu44["tu-consistency · 🔵"]
        end
    end
    BR43 --> W44

    subgraph BR44["🧪 БАРЬЕР 4.4 · Data Trust — integration → main"]
        direction LR
        x9["x9 data-trust<br/>GUARD+merge → e2e #21/#22 + полный регресс<br/>→ main (ff) · 🔴"]
    end
    W44 --> BR44

    subgraph W51["ВОЛНА 5.1 · Review Queue (§11)"]
        direction LR
        b30["b30 review-сервис + журнал · 🔵"]
        f26["f26 экран /validation · 🔵"]
        tu51["tu-review · 🔵"]
    end
    BR44 --> W51
    subgraph BR51["🚦 БАРЬЕР 5.1 — integration → main"]
        x10["x10 review-queue<br/>e2e #23 + регресс → main (ff) · 🔴"]
    end
    W51 --> BR51

    subgraph W52["ВОЛНА 5.2 · Coaching Loop (§12, синтетика)"]
        direction LR
        b31["b31 генератор датасета · 🔵"] --> b32["b32 coaching-сервис · 🔵"]
        f27["f27 секция отчёта + бейдж демо · 🔵"]
        tu52["tu-coaching · 🔵"]
    end
    BR51 --> W52
    subgraph BR52["🎓 БАРЬЕР 5.2 — integration → main"]
        x11["x11 coaching<br/>e2e #24 + регресс → main (ff) · 🔴"]
    end
    W52 --> BR52

    subgraph W53["ВОЛНА 5.3 · Driver Value (§13)"]
        direction LR
        b33["b33 позитив/green-zone · 🔵"] --> b34["b34 единый рейтинг · 🔵"]
        f28["f28 /leaderboard + блок позитива · 🔵"]
        tu53["tu-score · 🔵"]
    end
    BR52 --> W53
    subgraph BR53["🏆 БАРЬЕР 5.3 · ФИНАЛ В.5 — integration → main"]
        x12["x12 driver-value<br/>e2e #25/#26 + регресс → main (ff) · 🔴"]
    end
    W53 --> BR53
```
...
| 🧪 4.4 · Data Trust | `integration` → `main` | `x9-data-trust.md` | GUARD+merge → e2e консистентности (#21/#22, §10) + полный регресс → повторно продвигает `main` |
| 🚦 5.1 · Review Queue | `integration` → `main` | `x10-review-queue.md` | GUARD+merge → e2e очереди верификации (#23, §11) + полный регресс → продвигает `main` |
| 🎓 5.2 · Coaching | `integration` → `main` | `x11-coaching.md` | GUARD+merge → e2e цикла обучения (#24, §12) + байт-стабильность генератора + полный регресс → продвигает `main` |
| 🏆 5.3 · Driver Value (финал В.5) | `integration` → `main` | `x12-driver-value.md` | GUARD+merge → e2e скоринга (#25/#26, §13) + инвариант бленда + полный регресс → продвигает `main` |

> Файлы барьеров (по одному на волну): `barrier-1-p0/` (x1–x3) · `barrier-2-1-reports-voice/` (x4a) · `barrier-2-2-applied/` (x4b) · `barrier-2-3-tests/` (x4) · `barrier-3-hardening/` (x5) · `barrier-4-1-smart-context/` (x6) · `barrier-4-2-assistant/` (x7) · `barrier-4-3-ops-trust/` (x8) · `barrier-4-4-data-trust/` (x9) · `barrier-5-1-review-queue/` (x10) · `barrier-5-2-coaching/` (x11) · `barrier-5-3-driver-value/` (x12). Общие `x2`/`x3` живут в `barrier-1-p0/` и переиспользуются.
> **GUARD во всех барьерах, сливающих `feat/*`** (`x1`,`x2`,`x4a`,`x4b`,`x4`,`x5`,`x6`,`x7`,`x8`,`x9`,`x10`,`x11`,`x12`): стоп при незакоммиченном worktree.
> **Все барьеры (`x1`–`x12`) — 🔴 owner-only/high-judgement** (судят green/red, продвигают `main`, заводят дефекты); см. легенду сложности выше.
...
>
> ⚠️ **Гонка коммитов:** `b18∥b19∥b20` (окно 1) и 5× `tu-*` (окно 3) идут параллельно в одной ветке —
> каждый `## Коммит` стейджит **только свои файлы** (`git add <пути>`, не `-A`); роутеры автодискаверятся,
> `api/routers/__init__.py` не трогаем. Файлы дизъюнктны → конфликтов содержимого нет.
...
| Окно | Промпты | Примечание |
| --- | --- | --- |
| 1 Backend | `b21`🔴 ∥ `b22`🔵 (нарратив) ∥ `b23`🔵 (вердикт) | ⚠ роутер `b21` подхватывается автодискавери `api/main.py:_discover_routers` (модульный `router`; `ALL_ROUTERS` — легаси, не редактировать); `b23` — без роутера (аддитивная правка sabotage); `b22` — поле `narrative` (§7.5/§8.4) |
...
### Волна 4.4 — Data Trust (окна 1, 2, 3 · после зелёного x8)

Слой доверия к данным (§10, фичи #21/#22): кросс-датасетный валидатор + сверка скоростей.
Детерминированный SQL, без AI. Клиентское обоснование — Фомин (скорости), Маслов (дубли терминалов).

| Окно | Промпты | Примечание |
| --- | --- | --- |
| 1 Backend | **`b28`🔵 → `b29`🔵 (последовательно)** | b28 — `/api/consistency` (7 проверок, view `34_*.sql`); b29 — `/speed-check` (view `35_*.sql`) + замена CTE-заглушки в `34_*.sql` (согласовано, §10.6); роутеры — автодискавери, openapi-ассерты в Check |
| 2 Web | `f25`🔵 (после b29) | бейдж на `IncidentCard` (рядом с f15-чипом) + панель на `/metrics` (ниже f21); в UI «GPS-трек», не «CAN» (ASSUMPTION §10.2) |
| 3 Tests | `tu-consistency`🔵 (после b29, ∥ f25) | пороги/окно ±10с/инварианты/негативы `no_data` |

### Барьер 4.4 — Data Trust (основное окно `skai_7`)

git **внутри промпта** (`x9` GUARD+merge → e2e #21/#22 + полный регресс → `main` ff-only, паттерн x8):

```text
Выполни @prompts/v2-fullstack/barrier-4-4-data-trust/x9-data-trust.md
```

Красный барьер → **стоп**, дефект трека, `main` остаётся на финале Волны 4.

### Волна 5.1 — Review Queue (после зелёного x9)

| Окно | Промпты | Примечание |
| --- | --- | --- |
| 1 Backend | `b30`🔵 | журнал `output/review_queue.csv` (паттерн actions.csv, §11.0 — единый статусный словарь, actions.csv не дублировать); роутер — автодискавери, openapi-ассерт |
| 2 Web | `f26`🔵 (после b30) | живой экран на существующем `/validation` — ревизия допущения f22 (владелец сироты появился); `/response` не трогать |
| 3 Tests | `tu-review`🔵 (после b30, ∥ f26) | tmp-журнал, перезапись статуса, битые строки, изоляция от actions.csv |

### Барьер 5.1 (основное окно `skai_7`)

```text
Выполни @prompts/v2-fullstack/barrier-5-1-review-queue/x10-review-queue.md
```

### Волна 5.2 — Coaching Loop (после зелёного x10)

| Окно | Промпты | Примечание |
| --- | --- | --- |
| 1 Backend | **`b31`🔵 → `b32`🔵 (последовательно)** | b31 — детерминированный генератор `data/seed/training_assignments.csv` (crc32, загрузка существующим glob — ETL не трогаем) + строка в `make seed`; b32 — сервис/KPI (порог теста 18/20) |
| 2 Web | `f27`🔵 (после b32) | секция в driver-ветке отчёта; **бейдж «синтетические данные (демо)» обязателен** (§12.0) |
| 3 Tests | `tu-coaching`🔵 (после b32, ∥ f27) | байт-идентичность генератора, словарь курсов, KPI-инварианты |

### Барьер 5.2 (основное окно `skai_7`)

```text
Выполни @prompts/v2-fullstack/barrier-5-2-coaching/x11-coaching.md
```

### Волна 5.3 — Driver Value (после зелёного x11 · финал Волны 5)

| Окно | Промпты | Примечание |
| --- | --- | --- |
| 1 Backend | **`b33`🔵 → `b34`🔵 (последовательно)** | формулы зафиксированы §13 (веса не изобретать); b33 импортирует `enrichment.speed_limit_for`; b34 вызывает b33 и берёт risk_score из готовых инцидентов (§2 не копировать — урок b27) |
| 2 Web | `f28`🔵 (после b34) | `/leaderboard` + NAV + блок позитива в отчёте ПОСЛЕ секции f27 (волны последовательны — конфликта по Report.tsx нет); **дисклеймер периода обязателен** (§13.0) |
| 3 Tests | `tu-score`🔵 (после b34, ∥ f28) | ручной пересчёт формул, инвариант бленда, ТС без алармов, границы green_zone |

### Барьер 5.3 — финал Волны 5 (основное окно `skai_7`)

```text
Выполни @prompts/v2-fullstack/barrier-5-3-driver-value/x12-driver-value.md
```

Красный барьер любой подволны 5.x → **стоп**, дефект трека, `main` остаётся на предыдущем зелёном финале.
