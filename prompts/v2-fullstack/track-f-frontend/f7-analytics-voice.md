...
- **Запись/микрофон**: состояния `idle/recording/processing` видимы; отказ в доступе к микрофону (`getUserMedia` denied) или его отсутствие → понятная ошибка + предложение ввести запрос текстом (фолбэк не голос).
- **transcribe**: ошибка/таймаут STT → состояние ошибки + «Повторить»; низкая `confidence` → подсветить и предложить отредактировать текст вручную.
- **NLU пустой/ошибочный**: `query` вернул нераспознанное/ошибку → не открывать дашборд, показать сообщение + `[Исправить]` (вернуть текст в поле); пустой `report` (нет нарушений) → empty-state дашборда, не падение.
- **Дашборд loading/error**: скелет KPI/таблицы на время `query`; ошибка → состояние ошибки с ретраем.
- **fleet toggle**: «По ТС» при отсутствии данных `getVehicleReport(plate)` → empty/ошибка ветки, переключатель не ломает В-1.
- **Видео-панель**: маршрутизация канала строго DMS→ch5 / ADAS→ch1; `video_available===false` → placeholder + «Запросить архив».
- **Локали**: `lang` транскрипции (RU/KK/EN) из ответа `transcribe`; даты периодов/нарушений из UTC-`ts` в таймзоне парка; KPI-числа `tabular-nums`.
- **a11y модала**: `ConfirmationModal` — фокус-trap, Esc=`[Исправить]`/закрыть, возврат фокуса на инициатор; `VoiceButton` имеет `aria-label`/`aria-pressed` по состоянию записи; `VideoSlidePanel` фокусируется при открытии.
