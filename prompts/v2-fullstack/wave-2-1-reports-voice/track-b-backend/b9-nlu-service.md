...
- **Мусор/пустой/только-пробелы запрос:** `parse("")`, `parse("   ")`, `parse("asdf 123 !!!")` → безопасный дефолт `ReportQuery(kind="fleet", period_days=3)`, без исключения.
- **Парсинг госномера:** RU/KK-формат (напр. `А123ВС77`, `123ABC02`) → `kind="driver"`, `plate` нормализован; plate имеет приоритет над ФИО при одновременном наличии.
- **Парсинг ФИО:** «Нарушения Иванова за 3 дня» → `kind="driver"`, `driver_name` содержит «Иванов»; падежные окончания фамилии не ломают распознавание.
- **Период:** «за N дней/дня/день»→N, «неделю/за неделю»→7, «месяц»→30; нет упоминания → дефолт `3`; некорректное/огромное N → разумный clamp (не отрицательное, не падать).
- **fleet vs driver:** «отчёт по парку»/«по парку за неделю по ТС» → `kind="fleet"` (`view="vehicles"` при «по ТС/машинам», `"drivers"` при «по водителям»); «Нарушения Иванова за 3 дня» → `kind="driver"`.
- **Groq при заданном ключе, но сбое (сеть/невалидный JSON/ошибка валидации):** молчаливый уход в regex, итог — валидный `ReportQuery`, без необработанного исключения.
- **Согласованность веток:** на одном входе Groq и regex дают совместимую по схеме `ReportQuery`; неизвестные/лишние поля от LLM отсекаются валидацией Pydantic.
...
```bash
git add -A && git commit -m "b9: <что сделано>"
```
