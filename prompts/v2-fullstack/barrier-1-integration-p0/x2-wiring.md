...
Не начинай склейку, пока не подтвердишь, что x1 выполнен корректно:

- `grep -r "import streamlit" . --exclude-dir=.venv` — пусто.
- `grep -rn "from backend" api/ web/` — пусто.
- Реальные данные на месте: `datasets/`, `data/analysis/`, `output/`.

Не прошло — вернись к x1 и доисправь, прежде чем подключать роутеры.
