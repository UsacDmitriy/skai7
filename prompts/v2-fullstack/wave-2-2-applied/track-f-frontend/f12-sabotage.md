...
- **loading**: скелетон карточек/счётчика (стабильная высота), aria-busy на списке.
- **error** (сеть/5xx): блок ошибки + «Повторить» (повторный `getSabotage()`); компактная панель в Monitor тоже показывает ошибку, не пустоту.
- **empty** (`[]`): «Саботаж не обнаружен» (а не пустой блок); счётчик «0».
- **Нет видео** (`video_url` пустой/битый): плейсхолдер вместо плеера, метка «DMS перекрыта» сохраняется.
- **Действия**: на время `postAction` кнопка disabled + спиннер; успех → подтверждение/тост; ошибка → кнопка снова активна + сообщение (без потери карточки).
- **Граничные данные**: `speed_kmh=0`/`dms_dark=false` приходить не должны (фильтрует бэк), но виджет не падает и не показывает их как саботаж.
...
- Кнопки «Создать заявку» / «Уведомить HR» шлют `POST /api/actions`; в полёте — disabled+спиннер; ошибка отката не теряет карточку.
- Состояния loading / error («Повторить») / empty («Саботаж не обнаружен») / нет видео — обработаны в обоих местах встраивания.
- a11y: карточки/кнопки доступны с клавиатуры и фокусируемы; тёмный кадр имеет текстовую альтернативу; «машина едет · {speed} км/ч» доступно скринридеру (не только цвет); даты в локали/таймзоне.
...
```bash
git add -A && git commit -m "f12: <что сделано>"
```
