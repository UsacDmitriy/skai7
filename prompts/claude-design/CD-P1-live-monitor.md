...
Макет: тёмная тема (#0F172A), три зоны + top bar.
...
TOP BAR (h-12, bg #0F172A, border-bottom #1E3A8A):
  ...
  Справа: 🕐 04:17:33 (mono white) · Активные: 7 · 🔴 4 · 🟡 3 · 🟠 2 · Решено: 24
...
ЛЕВАЯ ПАНЕЛЬ (w-[300px], bg #0F172A, border-right #1E293B):
  ...
  5 карточек — каждая кликабельная:
    Карточка 1 (ВЫБРАННАЯ): border-l-4 #3B82F6, bg #1E293B
      "В345КМ 97" (pill bg #374151) + [📹 ВИДЕО] badge зелёный + "8:15 · сегодня"
      "Петров Д.С." (#94A3B8)
      "💥 Датчик удара" (белый, 13px) · чипы: "102 км/ч" [🔴] · "🌙 Ночь"
      Score: 72 (жёлтый, 20px bold, right)
    Карточки 2-5: аналогично, разные severity

ЦЕНТР: Leaflet карта (flex-1, h-full)
  Тёмный стиль (CSS filter: invert + hue-rotate)
  5 маркеров ТС с цветом по severity
  Выбранный маркер В345КМ 97 — увеличен, красный пульсирующий

ПРАВАЯ ПАНЕЛЬ (w-[420px], bg #FFFFFF):
  Заголовок карточки:
    "В345КМ 97" (16px bold) + badge [🔴 Критичный] + Score 72
    "Петров Д.С. · 💥 Датчик удара · 08:15"
  Два видео-плейхолдера стопкой (16:9, bg #0F172A, border rounded):
    [▶] CAM-01 ADAS — Передняя
    [▶] CAM-02 DMS — Салон
  Статус камер: CAM-01 ✅ CAM-02 ✅ CAM-03 ❌ Нет сигнала
  Мини-график скорости (Recharts LineChart, синяя линия, без осей)
  Кнопки:
    [🔍 Открыть карточку инцидента] — bg #1E3A8A text white w-full h-10 rounded
    [📹 Позвонить через камеру] — bg #0369A1 text white w-full h-10 rounded
    [📊 Аналитика] — border w-full h-10 rounded

НИЖНЯЯ СТРОКА (h-8, bg #0F172A):
  "Всего ТС: 47 · С событиями: 7 · Онлайн: 43 · CAM offline: 3"
  Система: 📡 GPS ✅ · 💾 CAN ✅ · 📷 CAM ⚠ 2/47
