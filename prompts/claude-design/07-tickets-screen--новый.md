...
| Р788ОО 52 / Тихонов М.С. | Камера офлайн | ⚙ Диагностика | — | 🔴 Новая ⏱**просрочено** | 14.05 | Открыть → |
...
**TABLE (flex-1, overflow-y auto):**
  ...
  Badges источника в колонке: [📹 ВА] фиолетовый · [⚡ Телематика] голубой · [⚡📹 Оба] зелёный · [⚙ Диагностика] серый
    Severity border (по статусу): Новая=#DC2626 · В работе=#EA580C · Проверена=#1E3A8A · Закрыта=#16A34A
  Строка `is_overdue=true` (дедлайн прошёл, статус ≠ Закрыта): bg #FEF2F2 (вся строка) + бейдж-оверлей «⏱ Просрочено» (bg #FEE2E2, text #991B1B, font-bold). Это НЕ отдельный статус, а флаг поверх любого незакрытого.
  Hover: bg #F8FAFC
