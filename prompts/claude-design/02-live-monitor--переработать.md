...
> 🎯 **Статус макета:** ✅ ГОТОВ (карта-герой реализована: Leaflet+OSM тёмная, правая панель сворачиваемая). Папка: `ui/02 Живой мониторинг/`
...
## ⚠️ Иерархия (КРИТИЧНО — не повторять дрейф старого макета)
**КАРТА — ГЕРОЙ экрана.** Она занимает основную площадь (CENTER, flex-1, ≥55% ширины). Это
обзор парка, а не разбор одного инцидента. Правая панель деталей — **вторичная и сворачиваемая**
(показывается только когда выбран маркер; по умолчанию свёрнута/узкая). Видео и графики на мониторе —
**компактное превью**, глубокий разбор (синхронные плееры, полный акселерометр) живёт в «Карточке инцидента»
(`/incidents/:id`) — НЕ дублировать её здесь. Клик по маркеру/карточке → мини-деталь → кнопка
«Открыть карточку» ведёт в полноценную карточку. Идея #4/#10: 1 `unit_id` = 1 маркер, ролевой режим.
