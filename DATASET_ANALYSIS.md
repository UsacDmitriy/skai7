...
> с дата-слоем ([backend/](backend/)) и промптами разработки ([prompts/](prompts/)).
...
Фактически «бэкенд» здесь = слой загрузки данных
[backend/data_loader.py](backend/data_loader.py), который читает CSV из [data/](data/) в
